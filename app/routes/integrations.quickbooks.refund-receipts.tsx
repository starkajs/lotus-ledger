...
import type { Route } from "./+types/integrations.quickbooks.refund-receipts";
...
import {
  ...
  parseQbRefundReceiptPresenceFilter,
  syncQuickBooksRefundReceipts,
  type QbRefundReceiptPresenceFilter,
  type QuickBooksRefundReceiptRecord,
} from "~/lib/quickbooks-refund-receipts.server";
import { requireUser } from "~/lib/session.server";

function receiptDetailHref(receiptId: string, returnTo: string) {
  const params = new URLSearchParams({ returnTo });
  return `/integrations/quickbooks/refund-receipts/${receiptId}?${params}`;
}
...
function QbStatusBadge({ receipt }: { receipt: QuickBooksRefundReceiptRecord }) {
...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "QuickBooks refund receipts — Lotus Ledger" },
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const presence = parseQbRefundReceiptPresenceFilter(
    url.searchParams.get("presence"),
  );
  return listQuickBooksRefundReceipts({ page, presence });
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    const result = await syncQuickBooksRefundReceipts({
      triggeredBy: "app",
      userId: user.id,
    });
...
function ReceiptHints({ receipt }: { receipt: QuickBooksRefundReceiptRecord }) {
...
export default function QuickBooksRefundReceiptsPage({
  loaderData,
  actionData,
    ...
    >
      {loaderData.connected && (
        <>
          ...
          <p className="mb-3 text-xs text-ink-muted">
            {total === 0
              ? "No refund receipts for this filter."
      ...
      {receipts.length === 0 ? (
        <p className="text-sm text-ink-muted">
          {presence === "active"
            ? "No active refund receipts. Refresh from QuickBooks, or check Removed in QuickBooks."
