...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    const result = await syncQuickBooksClasses({
      triggeredBy: "app",
      userId: user.id,
    });
...
export default function QuickBooksClassesPage({
  loaderData,
  actionData,
  ...
  return (
    <QuickBooksMasterDataPage
      title="Classes"
      description="QuickBooks classes synced into Lotus Ledger for departmental or project tracking."
      ...
      syncError={syncError}
      count={loaderData.classes.length}
      countLabel="classes"
    >
      {loaderData.classes.length === 0 ? (
        <p className="text-sm text-ink-muted">
          No classes stored yet. Click Refresh from QuickBooks to import.
        </p>
      ) : (
