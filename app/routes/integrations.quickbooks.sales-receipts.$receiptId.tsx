...
import type { Route } from "./+types/integrations.quickbooks.sales-receipts.$receiptId";
...
import { getQuickBooksSalesReceiptById } from "~/lib/quickbooks-sales-receipts.server";
...
export function meta({ data }: Route.MetaArgs) {
  const label = data?.receipt.docNumber ?? data?.receipt.quickbooksId ?? "Sales receipt";
  return [
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const receipt = await getQuickBooksSalesReceiptById(params.receiptId);
  if (!receipt) {
    throw new Response("Sales receipt not found", { status: 404 });
  }

  const url = new URL(request.url);
  const returnTo =
    url.searchParams.get("returnTo") ?? "/integrations/quickbooks/sales-receipts";
...
export default function QuickBooksSalesReceiptDetailPage({
  loaderData,
    ...
    >
      {receipt.qbStatus === "deleted_in_qb" && (
        ...
        >
          ...
          <p className="mt-1 text-ink-muted text-dark">
            This receipt no longer exists in QuickBooks (within the last 30-day
            sync window). Do not use it for Stripe reconciliation.
            {receipt.deletedInQbAt && (
              <>
                {" "}
                Detected{" "}
                {formatQuickBooksDateTime(receipt.deletedInQbAt)}.
              </>
            )}
          </p>
        </div>
      ...
      <div className="mt-4 rounded-jamyang-lg border border-sand-dark/50 bg-surface-overlay px-4 py-4 sm:px-6">
        ...
        <dl className="mt-3">
          ...
          <DetailRow label="Class ID">
            {receipt.classRefId ? (
              <span className="font-mono text-xs">{receipt.classRefId}</span>
            ) : (
              "—"
            )}
          </DetailRow>
          <DetailRow label="Department ID">
            {receipt.departmentRefId ? (
              <span className="font-mono text-xs">{receipt.departmentRefId}</span>
            ) : (
              "—"
            )}
          </DetailRow>
          ...
          <DetailRow label="Lotus created">
            {formatQuickBooksDateTime(receipt.createdAt)}
          </DetailRow>
          <DetailRow label="Lotus updated">
            {formatQuickBooksDateTime(receipt.updatedAt)}
          </DetailRow>
          ...
          <DetailRow label="Last seen in QB">
            {formatQuickBooksDateTime(receipt.lastSeenAt)}
          </DetailRow>
          <DetailRow label="Removed in QB at">
            {formatQuickBooksDateTime(receipt.deletedInQbAt)}
          </DetailRow>
          <DetailRow label="Internal ID">
            <span className="font-mono text-xs">{receipt.id}</span>
          </DetailRow>
          <DetailRow label="Realm ID">
            <span className="font-mono text-xs">{receipt.realmId}</span>
          </DetailRow>
        </dl>
      </div>
